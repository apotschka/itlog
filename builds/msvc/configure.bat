...
IF %1.==--help. (
    ...
    ECHO    --without-il_selftest   do not build il_selftest.exe
    GOTO :eof
)
ECHO Configuring itlog...
...
ECHO. >> platform.h
ECHO #define IL_HAVE_WINDOWS 1>> platform.h
...
IF EXIST "..\..\..\libzmq" (
    ...
    ECHO itlog cannot build without libzmq
...
IF EXIST "..\..\..\czmq" (
    ...
    ECHO itlog cannot build without czmq
...
:with_draft
    ...
    ECHO #define IL_BUILD_DRAFT_API 1>>platform.h
    GOTO end_draft
:no_draft
    ECHO Building without draft API (stable + legacy API)
    ECHO #undef IL_BUILD_DRAFT_API 1>>platform.h
:end_draft
ECHO. >> platform.h
